  ...
  return (
    <>
      ...
      <div>
        {SERVICES_DETAIL.map((service, index) => (
          <section
            key={service.id}
            id={service.id}
            className="py-28 border-b border-white/5 scroll-mt-20"
          >
            <div className="max-w-7xl mx-auto px-6 lg:px-10">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                ...
                <div className="lg:col-span-3">
                  <ScrollReveal>
                    <span className="font-syne font-800 text-7xl text-white/5 block mb-4">
                      0{index + 1}
                    </span>
                    <div className="w-8 h-px bg-brand mb-4" />
                    <h2 className="font-syne font-700 text-2xl tracking-tighter text-cream">
                      {service.title}
                    </h2>
                  </ScrollReveal>
                </div>
                    ...
                    <ul className="space-y-3">
                      {service.items.map((item) => (
                        <li key={item} className="flex items-start gap-3">
                          ...
                          <span className="font-dm text-sm text-accent leading-relaxed">{item}</span>
                        </li>
                      ))}
                    </ul>
                  </ScrollReveal>
      ...
      <section className="py-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <ScrollReveal>
            <div className="border border-brand/20 p-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
              <div>
                <span className="font-dm text-xs tracking-[0.3em] text-brand uppercase block mb-3">
                  Próximamente
                </span>
                <h2 className="font-syne font-700 text-2xl tracking-tighter text-cream mb-2">
                  BRUX-OUT
                </h2>
                <p className="font-dm text-sm text-accent max-w-md">
              ...
              <div className="shrink-0">
                <span className="font-dm text-xs text-brand tracking-wider border border-brand/30 px-4 py-2">
                  En desarrollo
                </span>
              </div>
            </div>
